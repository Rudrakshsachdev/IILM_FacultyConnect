// Enhanced file upload functionality
        document.addEventListener('DOMContentLoaded', function() {
            ...
            form.addEventListener('submit', function(e) {
                ...
                const awardDate = document.querySelector('input[name="date"]');
                if (awardDate.value) {
                    const award = new Date(awardDate.value);
                    const today = new Date();
                    
                    if (award > today) {
                        isValid = false;
                        awardDate.classList.add('is-invalid');
                        // Add error message
                        if (!awardDate.parentNode.querySelector('.date-error')) {
                            const errorDiv = document.createElement('div');
                            errorDiv.className = 'text-danger mt-1 date-error';
                            errorDiv.textContent = 'Award date cannot be in the future';
                            awardDate.parentNode.appendChild(errorDiv);
                        }
                    } else {
                        awardDate.classList.remove('is-invalid');
                        // Remove error message
                        const existingError = awardDate.parentNode.querySelector('.date-error');
                        if (existingError) {
