// Enhanced file upload functionality
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Form validation
            const form = document.getElementById('phdGuidanceForm');
            form.addEventListener('submit', function(e) {
                ...
                // Date validation: completion date should not be in the future
                const completionDate = document.querySelector('input[name="date_of_completion"]');
                if (completionDate.value) {
                    ...
                    if (completion > today) {
                        ...
                        // Add error message
                        if (!completionDate.parentNode.querySelector('.date-error')) {
                            const errorDiv = document.createElement('div');
                            errorDiv.className = 'text-danger mt-1 date-error';
                            errorDiv.textContent = 'Completion date cannot be in the future';
                            completionDate.parentNode.appendChild(errorDiv);
                        }
                    } else {
                        completionDate.classList.remove('is-invalid');
                        // Remove error message
                        const existingError = completionDate.parentNode.querySelector('.date-error');
                        if (existingError) {
