// Enhanced file upload functionality
        document.addEventListener('DOMContentLoaded', function() {
            ...
            form.addEventListener('submit', function(e) {
                ...
                // Amount validation
                const amountField = document.querySelector('input[name="amount_received"]');
                if (amountField.value) {
                    const amount = parseFloat(amountField.value);
                    if (amount < 0) {
                        isValid = false;
                        amountField.classList.add('is-invalid');
                        if (!amountField.parentNode.querySelector('.amount-error')) {
                            const errorDiv = document.createElement('div');
                            errorDiv.className = 'text-danger mt-1 amount-error';
                            errorDiv.textContent = 'Amount cannot be negative';
                            amountField.parentNode.appendChild(errorDiv);
                        }
                    } else {
                        amountField.classList.remove('is-invalid');
                        const existingError = amountField.parentNode.querySelector('.amount-error');
                        if (existingError) {
